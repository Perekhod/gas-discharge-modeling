...
int main() {
    // Для корректного отображения русским символов
    setlocale(LC_ALL, "Ru");

    // Создание файла дя записи координат места пересечения
    std::ofstream file("collisions.txt");

    std::cout << "Введите количество частиц:";
    int N{0};               // Количество частиц
    std::cin >> N;
    ...
    for (double t = 0; t < T; t += dt) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (p->cross(Lx, Ly, Lz, Sx, Sy, Sz, Snx, Sny, Snz, dt)) {
                ...
                file << " Частица #" << count_part << " прилипла к поверхности в точке (" << p->getX() << ", " << p->getY() << ", " << p->getZ() << ")\n";
                count_part++;
